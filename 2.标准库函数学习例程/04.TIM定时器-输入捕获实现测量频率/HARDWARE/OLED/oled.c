...
//只有调用OLED_Update函数或OLED_UpdateArea函数后才会将显存数组的数据发送到OLED硬件，进行显示
uint8_t OLED_DisplayBuf[8][128];

//写SCL高低电平，当参数传入0时，置SCL为低电平，当参数传入1时，置SCL为高电平
...
//写SDA高低电平，当参数传入0时，置SDA为低电平，当参数传入1时，置SDA为高电平
...
//OLED引脚初始化，将SCL和SDA引脚初始化为开漏模式，并释放引脚
void OLED_GPIO_Init(void)
{
	...
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;  //开漏模式
...
//OLED写数据，Data:要写入数据的起始地址，Count:要写入数据的数量
...
//设置显示光标位置，Page指定光标所在的页，范围：0~7，X指定光标所在的X轴坐标，范围：0~127
//OLED默认的Y轴，只能8个Bit为一组写入，即1页等于8个Y轴坐标
...
//判断指定点是否在指定多边形内部，1：在内部，0：不在内部
//nvert:多边形的顶点数,vertx、verty包含多边形顶点的x和y坐标的数组，testx、testy为测试点的x和y坐标
