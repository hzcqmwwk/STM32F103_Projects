...
2. **数据传递方式** ： FreeRTOS中队列采用实际值传递，即将数据拷贝到队列中进行传递，但是也可以传递指针，所以在传递较大的数据的时候采用指针传递。
...
1. 将该任务的状态列表项挂载在阻塞列表pxDelayedTaskList，由运行变为阻塞；
...
1. 将该任务的状态列表挂载在pxDelayedTaskList，由运行变为阻塞；
